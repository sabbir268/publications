...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_users` (`p_user_fname` VARCHAR(255), `p_user_lname` VARCHAR(255), `p_username` VARCHAR(255), `p_password` VARCHAR(255), `p_role` VARCHAR(255), OUT `p_msg` VARCHAR(100))  BEGIN 
	...
	START TRANSACTION;
	INSERT INTO users (first_name,last_name,username,password,role,created_at)
    VALUES (p_user_fname,p_user_lname,p_username,p_password,p_role,CURRENT_TIMESTAMP());
	COMMIT;
	SET p_msg='Successful Added';
END$$
